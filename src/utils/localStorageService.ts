...
export const getUserNetworkMembers = (userId: string): NetworkMember => {
  console.log(`Retrieving network members for user ID: ${userId}`);
  
  // Try to get user-specific network data first
  const userNetworkKey = `${STORAGE_KEYS.NETWORK_MEMBERS}_${userId}`;
  const userNetwork = getFromStorage<NetworkMember>(userNetworkKey);
  
  if (userNetwork) {
    console.log(`Found network data at key ${userNetworkKey} with ${userNetwork.children?.length || 0} children`);
    return userNetwork;
  }
...
export const getAllKycRequests = (): KycRequest[] => {
  return getFromStorage<KycRequest[]>(STORAGE_KEYS.KYC_REQUESTS) || [];
};

export const addKycRequest = (request: KycRequest): void => {
  const requests = getAllKycRequests();
  requests.push(request);
  setToStorage(STORAGE_KEYS.KYC_REQUESTS, requests);
};

export const updateKycRequest = (updatedRequest: KycRequest): void => {
  const requests = getAllKycRequests();
...
export const addNewUserWithData = (user: User): void => {
...
export const updateUserKycStatus = (userId: string, status: KYCStatus, reviewNotes?: string): boolean => {
  try {
    const users = getAllUsersForAdmin();
...
export const getKycDocuments = (): any[] => {
  try {
    const storedDocuments = localStorage.getItem('kycDocuments');
...
export const getKycSubmissionHistory = (): KycSubmission[] => {
  try {
...
export const addKycSubmission = (submission: KycSubmission): void => {
  try {
    const currentUser = getCurrentUser();
    if (!currentUser) return;
